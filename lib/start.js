...
if (process.env.ELECTRON_RUN_AS_NODE || process.env.ATOM_SHELL_INTERNAL_RUN_AS_NODE) {
  ...
  permissions.isElevated().then((isElevated) => {
    ...
    const MESSAGE = 'Etcher needs special permissions to interact with raw devices. Please provide your password.'

    return Bluebird.any(_.map([
      {
        name: 'gksudo',
        options: [
          '--preserve-env',
      ...
      return new Bluebird((resolve, reject) => {
        const executable = process.env.APPIMAGE ? [ process.env.APPIMAGE ] : process.argv
        let commandArgs = [ ...command.options, ...executable ]
        if (command.singleArgument) {
          commandArgs = [ commandArgs.join(' ') ]
        }
        const child = childProcess.spawn(command.name, commandArgs, {
          detached: true,
        ...
        // Forward any child process output and errors
      ...
      throw errors.createUserError({
        ...
        description: 'Please ensure you have either gksu, kdesudo, kdesu, or beesu available in your system'
      })
    })
  }).catch((error) => {
