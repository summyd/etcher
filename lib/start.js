    ...
    return Bluebird.any(_.map([
      {
        ...
        options: [
          '--preserve-env',
          '--message',
          'Please provide your password. Etcher needs special permissions to interact with raw devices.'
        ]
      },
      {
        name: 'kdesudo',
      ...
      return new Bluebird((resolve, reject) => {
        const args = process.env.APPIMAGE ? [ process.env.APPIMAGE ] : process.argv
        const appinstallSkip = !process.env.APPIMAGE ? {} : {
          SKIP: 1
        }
        const env = _.assign({}, process.env, appinstallSkip)
        const child = childProcess.spawn(command.name, _.concat(command.options, args), {
          detached: true,
          env
        })
      ...
      throw errors.createUserError({
        ...
        description: 'Please ensure you have either gksudo or kdesudo available in your system'
      })
    })
  }).catch((error) => {
